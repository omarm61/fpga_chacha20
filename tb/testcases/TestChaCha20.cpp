...
/** Configure Rx
****************************************************************************/
void CTestChaCha20::ConfigureRx(uint32_t u32KeyArray[], uint32_t u32NonceArray[])
{
  uint32_t rdata = 0;
  // --------------
  // Configure RX
  ...
  // Load Key
  for (size_t i = 0; i < 8; i++) {
    sim.AxiWrite(RX_AXI_CHACHA20_KEY1_OFFSET + (0x4*i), u32KeyArray[i]);
  }
  // Load Nonce
  for (size_t i = 0; i < 3; i++) {
    sim.AxiWrite(RX_AXI_CHACHA20_NONCE1_OFFSET + (0x4*i), u32NonceArray[i]);
  }
  sim.AxiSetBit(RX_AXI_CONTROL_OFFSET, RX_CONTROL_ENCRYPT_TYPE_INDEX, 1); // Enable ChaCha20 keystream
  sim.AxiSetBit(RX_AXI_CONTROL_OFFSET, RX_CONTROL_KEY_RELOAD_INDEX, 1); // Load Key
  sim.AxiSetBit(RX_AXI_CONTROL_OFFSET, RX_CONTROL_ENABLE_INDEX, 1); // Write Request
}
...
void CTestChaCha20::TestHelloWorld()
{
  ...
  sim.ValidateString(txMsg, rxMsg, "ChaCha20 Encryption: Hello World!");
}
...
void CTestChaCha20::RunTestSuite(uint32_t &seed)
{
  ...
  ConfigureRx(u32KeyArray, u32NonceArray);
